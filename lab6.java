...
public class lab6 extends GLJPanel implements GLEventListener {
	...
	public void display(GLAutoDrawable drawable) {
		...
		drawPolygonAndTriangle(gl2, 16, 5, 9);
		gl2.glPopMatrix();
	}

	public void drawBase(GL2 gl2, int n, int r) 
	{
	    gl2.glBegin(GL2.GL_POLYGON);
	    for (int i = 0; i < n; i++) 
	    {
	        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[15], 0);
	        double x = r * Math.cos(2 * i * Math.PI / n);
	        double z = r * Math.sin(2 * i * Math.PI / n);
	        gl2.glVertex3d(x, 1, z);
	    }
	    gl2.glEnd();
	}

	public void drawPolygonAndTriangle(GL2 gl2, int n, int r, int h)
	{
	    drawBase(gl2, n, r);
	    for (int i = 0; i < n; i++)
	    {
	        gl2.glBegin(GL2.GL_POLYGON);
	        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[15], 4);
	        double x1 = r * Math.cos(2 * i * Math.PI / n);
	        double z1 = r * Math.sin(2 * i * Math.PI / n);
	        gl2.glVertex3d(x1, 1, z1);
	        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[15], 8);
	        gl2.glVertex3d(0, h, 0);
	        i++;
	        double x2 = r * Math.cos(2 * i * Math.PI / n);
	        double z2 = r * Math.sin(2 * i * Math.PI / n);
	        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[15], 12);
	        gl2.glVertex3d(x2, 1, z2);
	        i--;
	        gl2.glEnd();
	    }
	}
